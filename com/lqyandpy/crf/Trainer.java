...
public class Trainer {
	...
	public void Train(ANN argA,double[][] argD,double argSC,int max_try){
		ArrayList<OutputNode> tempON=argA.getOutputNodes();//锟斤拷锟斤拷锟节碉拷
		ArrayList<InputNode>  tempIN=argA.getInputNodes();//锟斤拷锟斤拷锟节碉拷
		ArrayList<HiddenNode> tempHN=argA.getHiddenNodes();//锟斤拷锟斤拷锟斤拷诘锟�
		
		double[][] tempIM=new double[argD.length][tempIN.size()];//锟斤拷锟斤拷值Matrix
		double[][] tempOM=new double[argD.length][tempON.size()];//锟斤拷锟街礛atrix
		...
		while(true){
			...
			System.out.println("锟斤拷"+epoch+"锟斤拷训锟斤拷锟斤拷锟斤拷睿�"+tempMSE+"\r\n");
			
			
			if(!new Double(tempMSE).equals(Double.NaN)&&tempMSE<=argSC){
				System.out.println("锟斤拷"+epoch+"锟斤拷训锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷");
				break;
			}
	...
	public void OnePointTrain(ANN argA,double[] argD,double[] argO,double argSC){//argD
			...
			int i=0;
			for(OutputNode on:tempON){//锟斤拷锟斤拷锟斤拷锟斤拷诘锟斤拷delta
			...
			for(HiddenNode hn:tempHN){//锟斤拷锟斤拷锟斤拷锟斤拷诘锟斤拷delta
				ArrayList<Node> tempTN=argA.getNodesFrom(hn);//锟斤拷锟斤拷诘锟斤拷锟斤拷锟斤拷拥锟斤拷锟斤拷锟斤拷喜锟节碉拷
				double tempE=0;
				for(Node tn:tempTN){
